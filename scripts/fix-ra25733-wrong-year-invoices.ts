/**
 * Rebill Maria invoices denied on RA 25733 for wrong DOS year (2025 submitted, should be 2026).
 *
 * Denied lines:
 * - ZB71415 (NAWABI F) Jan 8, 2025 → 2026-01-08 — source invoice #851
 * - BM32698 (OUTEN J) Jan 9, 2025 → 2026-01-09 — source invoice #853
 *
 * Creates new BILLED/UNPAID rebill invoices with 2026 DOS (submittedAt=null, payPeriodId=null).
 * Renames Maria Drive session folders from 2025 labels to 2026 when needed.
 *
 * Usage: DOTENV_CONFIG_PATH=.env.local npx tsx -r dotenv/config scripts/fix-ra25733-wrong-year-invoices.ts [--fix]
 */
...
const REBILL_ROWS = [
  {
    sourceInvoiceNumber: 851,
    claim: "ZB71415",
    rebillServiceDate: "2026-01-08",
    targetFolderLabel: "01-08-2026",
  },
  {
    sourceInvoiceNumber: 853,
    claim: "BM32698",
    rebillServiceDate: "2026-01-09",
    targetFolderLabel: "01-09-2026",
  },
] as const;
...
async function main() {
  ...
  console.log("RA 25733 wrong-year rebill: create 2026 DOS invoices for ZB71415 + BM32698\n");
  ...
  const ra25733 = await prisma.remittanceAdvice.findFirst({
    where: { remittanceNumber: "25733" },
    include: {
      lines: {
  ...
  if (ra25733) {
    console.log("\nRA 25733 line matches" + (fix ? " after fix" : " (current)") + ":");
    for (const line of ra25733.lines) {
