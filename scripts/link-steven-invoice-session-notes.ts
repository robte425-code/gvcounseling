/**
 * Link Steven session clinical documents (SOAP, PHQ-4, GCPS, BHI forms) from the same
 * Drive session folder as each BILLED invoice PDF.
 *
 * Usage: npx tsx scripts/link-steven-invoice-session-notes.ts [--dry-run] [--use-cache]
 *   [--dos-claim-fallback]
 *
 * --use-cache: reuse OCR invoice numbers from rescan-steven-invoice-line-items-cache.json
 * --dos-claim-fallback: match by claim + session-folder date when invoice # OCR differs
 */
...
const RESULTS_PATH = "scripts/link-steven-invoice-session-notes-results.json";
const LOG_PATH = "scripts/link-steven-invoice-session-notes-run.log";
...
/** Clinically relevant session documents in Steven session folders. */
export function isStevenSessionDocument(filename: string, mimeType?: string): boolean {
  if (isStevenInvoiceFilename(filename)) return false;
...
async function collectSessionFolders(
  ...
  for (const session of sessionFolders) {
    ...
    const sessionDocs = files
      .filter((f) => isStevenSessionDocument(f.name, f.mimeType))
      .map((f) => ({
        filename: f.name,
        fileId: f.id,
...
async function main() {
  ...
  const byFolderKey = new Map<string, SessionFolderEntry>();
  for (const entry of folderEntries) {
    byFolderKey.set(folderKey(entry.claim, entry.sessionFolder), entry);
  }
