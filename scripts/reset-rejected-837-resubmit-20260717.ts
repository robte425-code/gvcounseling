/**
 * One-shot: after the second L&I 999 reject (REF*G2), flip the July 17
 * pay-period invoices that were re-billed back to SUBMITTED so a corrected
 * Production 837 can be regenerated. Skips PAID invoices (e.g. #263 ZB62154).
 *
 * Manual:
 *   FORCE_RESET_REJECTED_837_RESUBMIT=1 npx tsx scripts/reset-rejected-837-resubmit-20260717.ts
 */
...
const DONE_KEY = "reset_rejected_837_resubmit_20260717_done";
const REPORT_KEY = "reset_rejected_837_resubmit_20260717_report";
const NOTE_BODY =
  "Admin reset: workflow status returned from Billed → Submitted after L&I 999 rejected the resubmitted 2026-07-17 production 837 (rendering-provider REF*G2). Ready to regenerate Production (P) 837 after the generator fix.";

async function main() {
  if (!process.env.DATABASE_URL?.trim()) {
    console.log("reset-rejected-837-resubmit-20260717: DATABASE_URL not set — skipping");
    return;
  }
  ...
  const force = process.env.FORCE_RESET_REJECTED_837_RESUBMIT?.trim() === "1";
  if (!force) {
    ...
    if (done) {
      console.log("reset-rejected-837-resubmit-20260717: already completed — skipping");
  ...
  if (!payPeriod) {
    console.error(`reset-rejected-837-resubmit-20260717: no pay period with cutoff ${CUTOFF_ISO}`);
    await prisma.$disconnect();
    process.exitCode = 1;
    return;
  }
  ...
  // Keep already-paid RA invoices billed (e.g. #263 ZB62154).
  const toReset = billed.filter((inv) => inv.paymentStatus !== "PAID");
  const skippedPaid = billed.filter((inv) => inv.paymentStatus === "PAID");

  const lines: string[] = [];
  lines.push(`Reset resubmitted rejected-837 invoices for cutoff ${CUTOFF_ISO}`);
  ...
  if (skippedPaid.length) {
    ...
    for (const inv of skippedPaid) {
      lines.push(
        `  #${inv.invoiceNumber} ${inv.client.lniClaimNumber} ${inv.therapist.firstName} ${inv.therapist.lastName}` +
  ...
  console.log(
    `reset-rejected-837-resubmit-20260717: done reset=${toReset.length} skippedPaid=${skippedPaid.length}`,
  );
  await prisma.$disconnect();
}

main().catch((error) => {
  console.error("reset-rejected-837-resubmit-20260717: failed", error);
  process.exit(1);
});
