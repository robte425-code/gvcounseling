/**
 * Mark Steven invoices as therapist-paid when spreadsheet LNI Payment = Verified.
 *
 * Usage:
 *   DOTENV_CONFIG_PATH=.env.local npx tsx -r dotenv/config scripts/sync-steven-therapist-payments-from-spreadsheet.ts [--dry-run]
 */
...
const SPREADSHEET_PATH = "/Users/ghim/Downloads/Steven_ Client Billing Status.xlsx";
const SHEETS = ["Invoiced 2025", "Invoiced 2026"] as const;
const RESULTS_PATH = "scripts/sync-steven-therapist-payments-results.json";

const HISTORICAL_REMITTANCE_NUMBER = "STEVEN-SPREADSHEET";
...
function readVerifiedRows(): VerifiedRow[] {
  ...
  for (const sheetName of SHEETS) {
    ...
    for (const row of raw) {
      ...
      rows.push({ sheet: sheetName, invoiceNum, claim });
    }
  }
...
async function ensureHistoricalPayRun(adminId: string, therapistId: string, dryRun: boolean) {
  ...
  const remittance = existing
    ...
    : await prisma.remittanceAdvice.create({
        data: {
          ...
          payeeNumber: "0000000",
          payeeName: "Steven spreadsheet historical therapist payments",
          totalPaid: 0,
          status: "APPLIED",
          appliedAt: new Date(),
          sourceFilename: "Steven_ Client Billing Status.xlsx",
...
async function main() {
  ...
  for (const row of verifiedRows) {
    uniqueByInvoice.set(row.invoiceNum, row);
  }
  ...
  for (const row of uniqueByInvoice.values()) {
    const invoice = invoiceByNumber.get(row.invoiceNum);
    if (!invoice) {
      results.push({
  ...
  if (dryRun) {
    ...
    if (results.filter((r) => r.action === "missing").length) {
      console.log(
        "Missing:",
        results.filter((r) => r.action === "missing").slice(0, 20),
      );
    }
    writeFileSync(
      RESULTS_PATH,
  ...
  if (toCreate.length > 0) {
    const { payoutId } = await ensureHistoricalPayRun(admin.id, steven.id, false);
    ...
    const payout = await prisma.therapistPayRunPayout.findUniqueOrThrow({
      where: { id: payoutId! },
    ...
    await prisma.therapistPayRunPayout.update({
      where: { id: payoutId! },
      data: {
        invoiceCount: payout._count.lines,
    ...
    console.log(
      `Created ${toCreate.length} pay run line(s); totals this run: therapist $${therapistTotal.toFixed(2)}, LNI $${lniTotal.toFixed(2)}`,
    );
  }
